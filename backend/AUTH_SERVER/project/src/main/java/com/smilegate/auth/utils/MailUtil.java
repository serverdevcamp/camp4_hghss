...
public class MailUtil {
    ...
    @Value("${mail.signup.callback-url}")
    ...
    public boolean sendSignupMail(String key, User user){
        try {
            ...
            message.setContent(
                    ...
                    "<a href='"+signupCallbackUrl+"/users/signup/confirm?key=" + key + "' target='_blenk'>이메일 인증 확인</a>"
    ...
    public boolean sendPasswordMail(String key, String email) {
        try {
            ...
            message.setContent("<h1>[비밀번호 변경]</h1> <p>아래 링크를 클릭하시면 비밀번호를 변경하실 수 있습니다.</p> " +
                    "<a href='"+passwordCallbackUrl+"/password/change?key="
